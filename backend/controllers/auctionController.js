...
const createAuctionItem = async (req, res) => {
    const { title, description, startingBid, endDate } = req.body;
    const imagePath = req.file ? req.file.path : null; // Get the image path if it exists
    const userId = req.user.id;
    ...
    try {
        ...
        // Ensure the starting bid is a positive number
        if (startingBid <= 0) {
            return res.status(400).json({ message: "Starting bid must be a positive number" });
        }
        ...
        // Respond with the created auction item
        res.status(201).json(auctionItem);
    } catch (error) {
        console.error("Error creating auction item:", error);
        res.status(500).json({ message: "An error occurred while creating the auction item" });
    }
};
